...
template <size_t S> class NMEAParser {
	...
	public:
		...
		/*
		   ...
		   */
		void addHandler(const char *inToken, NMEAHandler inHandler, NMEAHandler inPreHandler = NULL)
		{
			if (mHandlerCount < S) {
				if (getHandler(inToken) == -1) {
					...
					mHandlers[mHandlerCount].mPreHandler = inPreHandler;
					mHandlerCount++;
				}
			}
		}
		...
		/*
		   ...
		   */
		void addHandler(const __FlashStringHelper *ifsh, NMEAHandler inHandler, NMEAHandler inPreHandler = NULL)
		{
			char buf[6];
			...
			addHandler(buf, inHandler, inPreHandler);
		}
#endif
			...
			switch (mState) {
				...
				case SENT:
					if (isalnum(inChar)) {
						...
						//I don't think resetting saves any time though
						mBuffer[mIndex] = 0;  //should be safe; it will be over-written

						//Look up 
						int8_t slot = getHandler(mBuffer);
						if (slot != -1) {
							if (mHandlers[slot].mPreHandler)
								mHandlers[slot].mPreHandler();
						} else {
							//TODO: this was supposed to save time
